...
const familyCases = [
    { 
        id: 1, 
        title: "收養檔案號：001", 
        content: "<strong>【高社經菁英家庭】</strong><br><strong>背景：</strong>先生（45歲）為科技業副總，太太（42歲）為外商銀行高管。年收破千萬，居住於市中心豪宅，並預先聘請全職保母。<br><strong>收養動機：</strong>經歷多年不孕，認為自身擁有頂級資源，能給予孩子最好的醫療與教育。<br><strong>支持系統：</strong>夫妻雙方皆在國外，無法請育嬰假，打算請保母帶。長輩傾向收養健康的小孩。" 
    },
    { 
        id: 2, 
        title: "收養檔案號：002", 
        content: "<strong>【雙薪彈性辦公家庭】</strong><br><strong>背景：</strong>先生（41歲）為科技業工程師，太太（39歲）為兼職會計。兩人收入中等但財務穩健。<br><strong>收養動機：</strong>結婚 8 年未生育，具備特殊兒早療志工經驗，理解孩子的進步不是直線的。<br><strong>支持系統：</strong>雙方長輩皆在外縣市，無法幫忙育兒，對於收養並無太大意見。先生工時長，目前正向公司爭取部分遠端辦公，太太會是主要照顧者。夫妻兩人已主動加入特殊兒家長互助社群，並確認住家附近有充足的早療資源與友善托育機構。" 
    },
    { 
        id: 3, 
        title: "收養檔案號：003", 
...
function buildEvalTable() {
  ...
  const accepted = gameState.choices
    ...
    .filter(Boolean);
  evalPages = accepted.length > 0 ? accepted : familyEvalData;
  evalPageIndex = 0;
  renderEvalPage();
}
...
function renderEvalPage() {
  const body = document.getElementById('eval-form-body');
  if (!body || evalPages.length === 0) return;

  const fam = evalPages[evalPageIndex];
  const total = evalPages.length;
  ...
  const rows = fam.rows.map(r =>
    `<tr>
      ...
      <td class="${r.cls}" style="white-space:nowrap;text-align:center;">${r.result}</td>
    </tr>`
  ).join('');

  const pager = total > 1 ? `
    <div class="eval-pager">
      <button class="eval-pager-btn" id="eval-prev" ${evalPageIndex === 0 ? 'disabled' : ''}>◀</button>
      <span>${evalPageIndex + 1} / ${total}</span>
      <button class="eval-pager-btn" id="eval-next" ${evalPageIndex === total - 1 ? 'disabled' : ''}>▶</button>
    </div>` : '';

  body.innerHTML = `
    <div>
      <div style="font-weight:bold;font-size:0.88em;margin-bottom:6px;text-align:left;">
        檔案號 ${fam.id}・${fam.label}
      </div>
      <table class="eval-table">${rows}</table>
      <div style="margin-top:6px;text-align:right;font-size:0.82em;">${verdict}</div>
      ${pager}
    </div>`;

  // 翻頁按鈕事件
  document.getElementById('eval-prev')?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (evalPageIndex > 0) { evalPageIndex--; renderEvalPage(); }
  });
  document.getElementById('eval-next')?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (evalPageIndex < evalPages.length - 1) { evalPageIndex++; renderEvalPage(); }
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.tooltip-container').forEach(container => {
    const tip = container.querySelector('.tooltip-text');
    if (!tip) return;
    container.addEventListener('click', (e) => {
      ...
      if (!isVisible) { tip.style.visibility = 'visible'; tip.style.opacity = '1'; }
    });
  });

  // 點畫面其他地方關閉所有 tooltip（手機版）
  ...
  // ====== 手機版：eval-form-popup 點擊關閉 ======
  document.getElementById('eval-form-popup')?.addEventListener('click', (e) => {
    if (!isMobile()) return;
    e.stopPropagation();
    document.getElementById('eval-form-popup').classList.add('hidden');
  });
  ...
  window.addEventListener('wheel', (e) => {
      if (isNavigating) return;
      if (currentSectionId === 'stage-1-result' && e.deltaY > 0) {
